...
public class GuildTestController : MonoBehaviour
{
    ...
    {
        return new EntityKey { Id = PlayFabLogin.ins.entityId, Type = PlayFabLogin.ins.entityType };
    }
    ...
    {
        var request = new ListMembershipRequest { Entity = GetEntity() };
    ...
    private void OnListGroups(ListMembershipResponse response)
    {
        ...
        foreach (var pair in response.Groups)
        {
            ...
            Debug.Log(pair.GroupName + " - " + pair.Group.Id + " - " + pair.Roles.First().RoleName);
        }
    }
    ...
    public void CreateGroup(string groupName)
    {
        ...
        var request = new CreateGroupRequest { GroupName = groupName, Entity = GetEntity() };
    ...
    {
        string groupID = EntityGroupPairs.First().Value;
        PlayFabClientAPI.GetAccountInfo(new PlayFab.ClientModels.GetAccountInfoRequest { PlayFabId = "6145C2D409938446" }, result =>
        {
            // A player-controlled entity invites another player-controlled entity to an existing group
            var request = new InviteToGroupRequest { Group = EntityKeyMaker(groupID), Entity = new EntityKey { Id = result.AccountInfo.TitleInfo.TitlePlayerAccount.Id, Type = result.AccountInfo.TitleInfo.TitlePlayerAccount.Type } };
